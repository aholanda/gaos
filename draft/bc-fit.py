...
def _tail(xs, xmin):
    '''Return an array with elements of x greater or equal than xmin. Used
    to avoid difference in python versions.
    ...
    Returns:
      xstail -  arrays with tailed elements.
    '''
    xstail = []
...
def pl(x):
    ...
    zetaV = sp.zeta(alphaV, 1) # TODO: remove 1
    constV = zetaV
    ...
    ntail = len(xtail)
    start_time = time.time()
    const = sp.zeta(alpha, 1) - np.sum(np.arange(1,xmin)**(-alpha))
...
def plpval(x, alpha, xmin, gof):
    ...
    print ('p = {:04.2f}   elapsed time = {:f}'.format(p, time.time()-starttime))
    return p
