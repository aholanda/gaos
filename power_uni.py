...
class PowerUni:
        ...
        def __init__(self, p1, p2, graph_name, verbose=False):
                ...
                self.G = graph.Graph('wr', PowerUni.dbname, graph_name, self.verbose)
                self.pa = pa.PrefAttach(self.G, True, False, self.verbose)
...
def usage(pname):
        ...
        \t                           from v by randomly and independently choosing w in proportion to 
        \t                           c/n where c > 1/n, in the current graph .
...
if __name__=='__main__':
        ...
        if len(sys.argv) == 2:
                if sys.argv[1] == '--reset-db':
                        graph.Graph.drop_and_create_tables(PowerUni.dbname)
                        exit(1)
        ...
        pu = PowerUni(p1, p2, graph_name, verbose)

        if p1 < 1.0 / pu.Glinux.no_vertices():
                print 'p1={} must be greater than 1/n={}'.format(p1, 1.0/pu.Glinux.no_vertices())
        ...
        print 'linux: {} vertices, {} arcs'.format(pu.Glinux.no_vertices(), pu.Glinux.no_arcs())
        print 'power_uni: {} vertices, {} arcs'.format(G.no_vertices(), G.no_arcs())
        print 'p1={}, p2={}'.format(pu.p1, pu.p2)
